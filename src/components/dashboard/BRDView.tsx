...
export default function BRDView() {
  ...
  const loadFromStorage = () => {
    ...
    if (stored) setSections(parseBRDText(stored));
    setIsDemo(mode === 'demo');
    setLoading(false);
  };
  ...
  useEffect(() => {
    loadFromStorage();
    // Re-render when AI assistant injects new content
    window.addEventListener('brd_updated', loadFromStorage);
    return () => window.removeEventListener('brd_updated', loadFromStorage);
  }, []); // loadFromStorage is defined in component scope; empty deps is intentional

  // ── PDF Export ──────────────────────────────────────────────────────────────
  const handleExportPDF = () => {
    const stored = sessionStorage.getItem('brd_text');
    if (!stored) {
      alert('No BRD content found. Please generate a BRD first.');
      return;
    }

    const parsed = parseBRDText(stored);
    ...
    const lineH = 16; // line height in pt

    let y = 60;

    // ── helper: check page break ────────────────────────────────────────────
    const checkPageBreak = (needed = lineH) => {
    ...
    const addWrappedText = (text: string, x: number, fontSize: number, color: [number, number, number]) => {
      doc.setFontSize(fontSize);
      doc.setTextColor(...color);
      const lines: string[] = doc.splitTextToSize(text, contentWidth - (x - marginL));
    ...
    doc.roundedRect(marginL, 40, contentWidth, 80, 8, 8, 'F');

    doc.setFontSize(24);
    doc.setTextColor(255, 255, 255);
    doc.setFont('helvetica', 'bold');
    const titleText = 'Business Requirements Document';
    const titleW = doc.getTextWidth(titleText);
    doc.text(titleText, (pageWidth - titleW) / 2, 88);

    doc.setFontSize(11);
    doc.setTextColor(180, 180, 220);
    doc.setFont('helvetica', 'normal');
    const subText = `Generated by Structify  ·  ${new Date().toLocaleDateString('en-US', { year: 'numeric', month: 'long', day: 'numeric' })}`;
    const subW = doc.getTextWidth(subText);
    doc.text(subText, (pageWidth - subW) / 2, 108);
    ...
    interface SectionConfig {
      ...
      label: string;
      number: number;
      type: 'paragraph' | 'bullets' | 'stakeholders' | 'requirements';
    }

    const sectionDefs: SectionConfig[] = [
      { number: 1, key: 'Executive Summary',         label: 'Executive Summary',         type: 'paragraph'     },
      { number: 2, key: 'Business Objectives',       label: 'Business Objectives',       type: 'bullets'       },
      { number: 3, key: 'Stakeholder Analysis',      label: 'Stakeholder Analysis',      type: 'stakeholders'  },
      { number: 4, key: 'Functional Requirements',   label: 'Functional Requirements',   type: 'requirements'  },
      { number: 5, key: 'Non-Functional Requirements', label: 'Non-Functional Requirements', type: 'bullets'   },
      { number: 6, key: 'Assumptions',               label: 'Assumptions',               type: 'bullets'       },
      { number: 7, key: 'Risks',                     label: 'Risks',                     type: 'bullets'       },
      { number: 8, key: 'Timeline',                  label: 'Timeline',                  type: 'bullets'       },
      { number: 9, key: 'Success Metrics',           label: 'Success Metrics',           type: 'bullets'       },
    ];

    sectionDefs.forEach(({ key, label, number, type }) => {
      const raw = parsed[key];
      if (!raw || raw.trim() === '') return;

      // Section heading with coloured bar
      checkPageBreak(40);
      doc.setFillColor(99, 102, 241); // indigo-500
      doc.rect(marginL, y - 12, 4, 22, 'F');

      doc.setFontSize(15);
      doc.setFont('helvetica', 'bold');
      doc.setTextColor(20, 20, 40);
      doc.text(`${number}. ${label}`, marginL + 12, y);
      y += 20;
      ...
      // Content
      if (type === 'paragraph') {
        addWrappedText(raw, marginL, 10.5, [50, 50, 70]);
        ...
        items.forEach((item) => {
          ...
          doc.circle(marginL + 5, y - 4, 2.5, 'F');
          addWrappedText(item, marginL + 16, 10.5, [50, 50, 70]);
        });

      } else if (type === 'stakeholders') {
        const stakeList = parseStakeholders(raw);
        if (stakeList.length > 0) {
          // Table header
          const colW = contentWidth / 4;
          const headers = ['Name', 'Role', 'Influence', 'Interest'];
          doc.setFillColor(240, 240, 250);
          doc.rect(marginL, y - 12, contentWidth, 20, 'F');
          doc.setFontSize(10);
          doc.setFont('helvetica', 'bold');
          doc.setTextColor(50, 50, 90);
          headers.forEach((h, hi) => doc.text(h, marginL + hi * colW + 6, y));
          y += 14;

          stakeList.forEach((s, si) => {
            checkPageBreak(20);
            if (si % 2 === 0) {
              doc.setFillColor(250, 250, 255);
              doc.rect(marginL, y - 12, contentWidth, 18, 'F');
            }
            doc.setFont('helvetica', 'normal');
            doc.setFontSize(10);
            doc.setTextColor(50, 50, 70);
            const row = [s.name, s.role, s.influence, s.interest];
            row.forEach((cell, ci) => doc.text(cell.substring(0, 28), marginL + ci * colW + 6, y));
            y += 18;
          });
        }

      } else if (type === 'requirements') {
        const reqs = extractRequirements(raw);
        reqs.forEach((req) => {
          checkPageBreak(40);
          doc.setFillColor(245, 245, 255);
          doc.roundedRect(marginL, y - 12, contentWidth, req.detail ? 38 : 24, 4, 4, 'F');

          doc.setFontSize(9);
          doc.setFont('helvetica', 'bold');
          doc.setTextColor(99, 102, 241);
          doc.text(req.id, marginL + 8, y);

          doc.setFontSize(10.5);
          doc.setFont('helvetica', 'bold');
          doc.setTextColor(20, 20, 40);
          const titleLines: string[] = doc.splitTextToSize(req.title, contentWidth - 60);
          doc.text(titleLines[0], marginL + 55, y);
          y += 16;
          ...
          if (req.detail) {
            ...
            const detLines: string[] = doc.splitTextToSize(req.detail, contentWidth - 16);
            detLines.forEach((dl: string) => {
              doc.text(dl, marginL + 8, y);
              y += 13;
            });
          } else {
            y += 4;
          }
          y += 8; // gap between requirements
        });
      }

      y += 18; // gap after section
    });
    ...
    for (let p = 1; p <= totalPages; p++) {
      ...
      doc.setFont('helvetica', 'normal');
      doc.text('Structify · Confidential', marginL, pageHeight - 24);
      const pgText = `Page ${p} of ${totalPages}`;
      doc.text(pgText, pageWidth - marginR - doc.getTextWidth(pgText), pageHeight - 24);
      // footer line
  ...
  return (
    <div className="max-w-5xl mx-auto pb-24 relative">
      ...
      <div className="sticky top-0 z-30 flex items-center justify-between bg-[#0a0a10]/80 backdrop-blur-md py-4 px-6 rounded-2xl border border-white/10 mb-8 shadow-2xl flex-wrap gap-3">
        ...
        <div className="flex items-center space-x-3">
          ...
          <button
            ...
            className="flex items-center space-x-2 text-sm font-medium text-white bg-gradient-to-r from-brand-600 to-purple-600 hover:from-brand-500 hover:to-purple-500 px-4 py-2 rounded-lg transition-colors shadow-lg shadow-brand-500/20"
          >
            <Download className="w-4 h-4" />
            <span>Download BRD as PDF</span>
          </button>
        </div>
      </div>
      ...
      <AnalyticsDashboard
        funcReqCount={funcReqs.length}
        nonFuncReqCount={nonFuncReqs.length}
